...
// Post Category
function pastelinterior_post_cats() {
...
// Post Tags
function pastelinterior_post_tags(){
...
// social media
if ( ! function_exists( 'pastelinterior_social' ) ) {
	function pastelinterior_social( $args = array()  ){
		...
		$url = pastelinterior_opt('pastelinterior_social_url');
...
function pastelinterior_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'pastel-interior' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'pastel-interior' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'pastel-interior' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function pastelinterior_set_post_views($postID) {
    $count_key = 'pastelinterior_post_views_count';
...
// blog post categoty 
function pastelinterior_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'pastel-interior' ) );
